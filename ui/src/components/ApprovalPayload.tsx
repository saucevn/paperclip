...
function SkillList({ values }: { values: unknown }) {
  ...
  return (
    <div className="flex items-start gap-2">
      <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("payload.fields.skills")}</span>
...
export function HireAgentPayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <div className="flex items-center gap-2">
        <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("payload.fields.name")}</span>
        <span className="font-medium">{String(payload.name ?? "—")}</span>
      </div>
      <PayloadField label={t("payload.fields.role")} value={payload.role} />
      <PayloadField label={t("payload.fields.title")} value={payload.title} />
      <PayloadField label={t("payload.fields.icon")} value={payload.icon} />
      {!!payload.capabilities && (
        <div className="flex items-start gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs pt-0.5">{t("payload.fields.capabilities")}</span>
          <span className="text-muted-foreground">{String(payload.capabilities)}</span>
        </div>
      )}
      {!!payload.adapterType && (
        <div className="flex items-center gap-2">
          <span className="text-muted-foreground w-20 sm:w-24 shrink-0 text-xs">{t("payload.fields.adapter")}</span>
...
export function BudgetOverridePayload({ payload }: { payload: Record<string, unknown> }) {
  ...
  return (
    <div className="mt-3 space-y-1.5 text-sm">
      <PayloadField label={t("payload.fields.scope")} value={payload.scopeName ?? payload.scopeType} />
      <PayloadField label={t("payload.fields.window")} value={payload.windowKind} />
      <PayloadField label={t("payload.fields.metric")} value={payload.metric} />
      {(budgetAmount !== null || observedAmount !== null) ? (
        <div className="rounded-md bg-muted/40 px-3 py-2 text-xs text-muted-foreground">
          {t("payload.budgetDisplay", {
    ...
    <div className="mt-4 space-y-3.5 text-sm">
      {title && (
        <div className="space-y-1">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("payload.fields.title")}</p>
          <p className="font-medium leading-6 text-foreground">{title}</p>
        </div>
      )}
      {summary && (
        <div className="space-y-1">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("payload.sections.summary")}</p>
      ...
      {recommendedAction && (
        <div className="rounded-lg border border-amber-500/20 bg-amber-500/10 px-3.5 py-3">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-amber-700 dark:text-amber-300">
            {t("payload.sections.recommendedAction")}
          </p>
      ...
      {nextActionOnApproval && (
        <div className="rounded-lg border border-border/60 bg-background/60 px-3.5 py-3">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("payload.sections.onApproval")}</p>
          <p className="mt-1 leading-6 text-foreground">{nextActionOnApproval}</p>
        </div>
      )}
      {risks.length > 0 && (
        <div className="space-y-1.5">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">{t("payload.sections.risks")}</p>
      ...
      {proposedComment && (
        <div className="space-y-1.5">
          <p className="text-[11px] font-medium uppercase tracking-[0.08em] text-muted-foreground">
            {t("payload.sections.proposedComment")}
          </p>
