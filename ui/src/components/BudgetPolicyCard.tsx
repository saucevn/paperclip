...
export function BudgetPolicyCard({
  ...
  const windowLabel =
    summary.windowKind === "lifetime" ? t("policy.lifetimeBudget") : t("policy.monthlyBudget");
  ...
  const observedBudgetGrid = isPlain ? (
    <div className="grid gap-6 sm:grid-cols-2">
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("policy.observed")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0
            ? `${summary.utilizationPercent}% ${t("policy.noCapConfigured")}`
            : t("policy.noCapConfigured")}
        </div>
      </div>
      <div>
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("policy.budget")}</div>
        <div className="mt-2 text-xl font-semibold tabular-nums">
          {summary.amount > 0 ? formatCents(summary.amount) : t("policy.disabled")}
        </div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.paused && summary.pauseReason
            ? t("policy.softAlertPause", { percent: summary.warnPercent, reason: summary.pauseReason })
            : t("policy.softAlertAt", { percent: summary.warnPercent })}
        </div>
      </div>
    </div>
  ) : (
    <div className="grid gap-3 sm:grid-cols-2">
      <div className="rounded-xl border border-border/70 bg-black/[0.18] px-4 py-3">
        ...
        <div className="mt-2 text-xl font-semibold tabular-nums">{formatCents(summary.observedAmount)}</div>
        <div className="mt-1 text-xs text-muted-foreground">
          {summary.amount > 0 ? `${summary.utilizationPercent}% of limit` : t("policy.noCapConfigured")}
        </div>
      </div>
      <div className="rounded-xl border border-border/70 bg-black/[0.18] px-4 py-3">
        <div className="text-[11px] uppercase tracking-[0.18em] text-muted-foreground">{t("policy.budget")}</div>
  ...
  const progressSection = (
    <div className="space-y-2">
      <div className="flex items-center justify-between text-xs text-muted-foreground">
        <span>{t("policy.remaining")}</span>
        <span>{summary.amount > 0 ? formatCents(summary.remainingAmount) : t("policy.unlimited")}</span>
      </div>
    ...
    <div className="flex items-start gap-2 rounded-xl border border-red-500/30 bg-red-500/10 px-3 py-2 text-sm text-red-100">
      ...
      <div>
        {summary.scopeType === "project"
          ? t("policy.pausedProject")
          : t("policy.pausedScope")}
      </div>
    </div>
  ) : null;
  ...
  const saveSection = onSave ? (
    <div className={cn("flex flex-col gap-3 sm:flex-row sm:items-end", isPlain ? "" : "rounded-xl border border-border/70 bg-background/50 p-3")}>
      ...
      >
        {isSaving ? t("policy.saving") : summary.amount > 0 ? t("policy.updateBudget") : t("policy.setBudget")}
      </Button>
    </div>
  ) : null;
  ...
  if (isPlain) {
    return (
      <div className="space-y-6">
        ...
        {saveSection}
        {parsedDraft === null ? (
          <p className="text-xs text-destructive">{t("policy.invalidAmount")}</p>
        ) : null}
      </div>
    );
  }
