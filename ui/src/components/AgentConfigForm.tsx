    ...
    <div className={cn("relative", cards && "space-y-6")}>
      ...
      {!isCreate && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agents:form.sections.identity")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agents:form.sections.identity")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field label={t("agents:form.fields.name")} hint={help.name}>
              <DraftInput
            ...
            <Field label={t("agents:form.fields.title")} hint={help.title}>
              <DraftInput
                value={eff("identity", "title", props.agent.title ?? "")}
                onCommit={(v) => mark("identity", "title", v || null)}
                immediate
                className={inputClass}
                placeholder={t("agents:form.fields.titlePlaceholder")}
              />
            </Field>
            <Field label={t("agents:form.fields.reportsTo")} hint={help.reportsTo}>
              <ReportsToPicker
            ...
            <Field label={t("agents:form.fields.capabilities")} hint={help.capabilities}>
              <MarkdownEditor
                value={eff("identity", "capabilities", props.agent.capabilities ?? "") ?? ""}
                onChange={(v) => mark("identity", "capabilities", v || null)}
                placeholder={t("agents:form.fields.capabilitiesPlaceholder")}
            ...
            {isLocal && !props.hidePromptTemplate && (
              <>
                <Field label={t("agents:form.fields.promptTemplate")} hint={help.promptTemplate}>
                  <MarkdownEditor
                    value={eff(
                      "adapterConfig",
                      "promptTemplate",
                    ...
                    placeholder={t("agents:form.fields.promptTemplatePlaceholder")}
                ...
                <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                  {t("agents:form.promptTemplateWarning")}
                </div>
              </>
            )}
          </div>
      ...
      {environmentsEnabled ? (
        <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agents:form.sections.execution")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agents:form.sections.execution")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <Field
              label={t("agents:form.fields.defaultEnvironment")}
            ...
            >
              ...
              >
                <option value="">{t("agents:form.fields.defaultEnvironmentPlaceholder")}</option>
      ...
      <div className={cn(!cards && (isCreate ? "border-t border-border" : "border-b border-border"))}>
        <div className={cn(cards ? "flex items-center justify-between mb-3" : "px-4 py-2 flex items-center justify-between gap-2")}>
          {cards
            ? <h3 className="text-sm font-medium">{t("agents:form.sections.adapter")}</h3>
            : <span className="text-xs font-medium text-muted-foreground">{t("agents:form.sections.adapter")}</span>
          }
          {showAdapterTestEnvironmentButton && (
            ...
            >
              {testEnvironment.isPending ? t("agents:form.adapterTest.testing") : t("agents:form.adapterTest.test")}
            </Button>
          )}
        </div>
        <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
          {showAdapterTypeField && (
            <Field label={t("agents:form.fields.adapterType")} hint={help.adapterType}>
              <AdapterTypeDropdown
                value={adapterType}
          ...
          {testEnvironment.error && (
            <div className="rounded-md border border-destructive/30 bg-destructive/10 px-3 py-2 text-xs text-destructive">
              {testEnvironment.error instanceof Error
                ...
                : t("agents:form.adapterTest.failed")}
            </div>
          )}
          ...
          {showLegacyWorkingDirectoryField && (
            <Field label={t("agents:form.fields.workingDir")} hint={help.cwd}>
              <div className="flex items-center gap-2 rounded-md border border-border px-2.5 py-1.5">
                ...
                <DraftInput
                  ...
                  placeholder={t("agents:form.fields.workingDirPlaceholder")}
                />
                <ChoosePathButton />
              </div>
            </Field>
          ...
          {isLocal && isCreate && (
            <>
              ...
              <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-100">
                {t("agents:form.promptTemplateCreateWarning")}
              </div>
            </>
          )}
      ...
      {isLocal && (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium mb-3">{t("agents:form.sections.permissions")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground">{t("agents:form.sections.permissions")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
              <Field label={t("agents:form.fields.command")} hint={help.localCommand}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.command
                  ...
                  {adapterType === "codex_local" &&
                    ...
                    currentThinkingEffort === "minimal" && (
                      <p className="text-xs text-amber-400">
                        {t("agents:form.codexMinimalThinkingWarning")}
                      </p>
                    )}
                </>
              )}
              {!isCreate && typeof config.bootstrapPromptTemplate === "string" && config.bootstrapPromptTemplate && (
                <>
                  <Field label={t("agents:form.fields.bootstrapPrompt")} hint={help.bootstrapPrompt}>
                    <MarkdownEditor
                      value={eff(
                        "adapterConfig",
                      ...
                      placeholder={t("agents:form.fields.bootstrapPromptPlaceholder")}
                      contentClassName="min-h-[44px] text-sm font-mono"
                      imageUploadHandler={async (file) => {
                  ...
                  <div className="rounded-md border border-amber-500/25 bg-amber-500/10 px-3 py-2 text-xs text-amber-200">
                    {t("agents:form.bootstrapPromptWarning")}
                  </div>
                </>
              )}
              ...
              <Field label={t("agents:form.fields.extraArgs")} hint={help.extraArgs}>
                <DraftInput
                  value={
                    isCreate
                      ? val!.extraArgs
                  ...
                  immediate
                  className={inputClass}
                  placeholder={t("agents:form.fields.extraArgsPlaceholder")}
                />
              </Field>

              <Field label={t("agents:form.fields.envVars")} hint={help.envVars}>
                <EnvVarEditor
                  value={
                    isCreate
              ...
              {!isCreate && (
                <>
                  <Field label={t("agents:form.fields.timeoutSec")} hint={help.timeoutSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "timeoutSec",
                  ...
                  <Field label={t("agents:form.fields.graceSec")} hint={help.graceSec}>
                    <DraftNumberInput
                      value={eff(
                        "adapterConfig",
                        "graceSec",
      ...
      {isCreate && showCreateRunPolicySection ? (
        <div className={cn(!cards && "border-b border-border")}>
          {cards
            ? <h3 className="text-sm font-medium flex items-center gap-2 mb-3"><Heart className="h-3 w-3" /> {t("agents:form.sections.runPolicy")}</h3>
            : <div className="px-4 py-2 text-xs font-medium text-muted-foreground flex items-center gap-2"><Heart className="h-3 w-3" /> {t("agents:form.sections.runPolicy")}</div>
          }
          <div className={cn(cards ? "border border-border rounded-lg p-4 space-y-3" : "px-4 pb-3 space-y-3")}>
            <ToggleWithNumber
              label={t("agents:form.fields.heartbeatOnInterval")}
              hint={help.heartbeatInterval}
              checked={val!.heartbeatEnabled}
              onCheckedChange={(v) => set!({ heartbeatEnabled: v })}
              number={val!.intervalSec}
              onNumberChange={(v) => set!({ intervalSec: v })}
              numberLabel={t("agents:form.fields.heartbeatSec")}
              numberPrefix={t("agents:form.fields.heartbeatEvery")}
        ...
        <div className={cn(!cards && "border-b border-border")}>
          ...
          <div className={cn(cards ? "border border-border rounded-lg overflow-hidden" : "")}>
            ...
            <CollapsibleSection
              title={t("agents:form.advancedRunPolicy")}
              bordered={cards}
            ...
            <div className="space-y-3">
              <ToggleField
                label={t("agents:form.fields.wakeOnDemand")}
              ...
              />
              <Field label={t("agents:form.fields.cooldownSec")} hint={help.cooldownSec}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "cooldownSec",
              ...
              <Field label={t("agents:form.fields.maxConcurrentRuns")} hint={help.maxConcurrentRuns}>
                <DraftNumberInput
                  value={eff(
                    "heartbeat",
                    "maxConcurrentRuns",
...
function AdapterEnvironmentResult({ result }: { result: AdapterEnvironmentTestResult }) {
  ...
  const statusLabel =
    result.status === "pass" ? t("form.adapterTest.passed") : result.status === "warn" ? t("form.adapterTest.warnings") : t("form.adapterTest.failed");
  const statusClass =
    result.status === "pass"
      ...
      <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
        {adapterList.map((item) => (
          ...
          >
            ...
            {item.comingSoon && (
              <span className="text-[10px] text-muted-foreground">{t("form.comingSoon")}</span>
            )}
          </button>
        ))}
      </PopoverContent>
...
function ModelDropdown({
  ...
  return (
    <Field label={t("form.fields.model")} hint={help.model}>
      <Popover
        open={open}
        ...
        <PopoverTrigger asChild>
          <button type="button" className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>
              {selected
                ? selected.label
                : value || (allowDefault ? t("form.fields.modelDefault") : required ? t("form.fields.modelSelectRequired") : t("form.fields.modelSelect"))}
            </span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
          <div className="relative mb-1">
            <input
              ...
              placeholder={creatable ? t("form.fields.modelSearchPlaceholder") : t("form.fields.modelSearchPlaceholderBasic")}
              value={modelSearch}
          ...
          {onDetectModel && !modelSearch.trim() && (
            ...
            >
              ...
              {detectingModel ? t("form.detectingModel") : detectedModel ? (detectModelLabel?.replace(/^Detect\b/, "Re-detect") ?? t("form.redetectModel")) : (detectModelLabel ?? t("form.detectModel"))}
            </button>
          )}
          {onRefreshModels && !modelSearch.trim() && (
            ...
            >
              ...
              {refreshingModels ? t("form.refreshingModels") : t("form.refreshModels")}
            </button>
          )}
          ...
          <div className="max-h-[240px] overflow-y-auto">
            ...
            {canCreateManualModel && (
              ...
              >
                <span>{t("form.fields.useManualModel")}</span>
            ...
            {filteredModels.length === 0 && !canCreateManualModel && promotedModelIds.size === 0 && (
              <div className="px-2 py-2 space-y-2">
                <p className="text-xs text-muted-foreground">
                  {onDetectModel
                    ? (emptyDetectHint ?? t("form.noModelDetected"))
...
function ThinkingEffortDropdown({
  ...
  return (
    <Field label={t("form.fields.thinkingEffort")} hint={help.thinkingEffort}>
      <Popover open={open} onOpenChange={onOpenChange}>
        <PopoverTrigger asChild>
          <button className="inline-flex items-center gap-1.5 rounded-md border border-border px-2.5 py-1.5 text-sm hover:bg-accent/50 transition-colors w-full justify-between">
            <span className={cn(!value && "text-muted-foreground")}>{selected?.label ?? options[0]?.label ?? "Auto"}</span>
            <ChevronDown className="h-3 w-3 text-muted-foreground" />
          </button>
        </PopoverTrigger>
        <PopoverContent className="w-[var(--radix-popover-trigger-width)] p-1" align="start">
