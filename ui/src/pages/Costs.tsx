    ...
    <Card>
      ...
      <CardContent className="grid gap-3 px-5 pb-5 pt-2 sm:grid-cols-2 xl:grid-cols-4">
        <MetricTile
          ...
          subtitle={t("financeSummary.debitsSubtitle", { count: eventCount })}
          icon={ArrowUpRight}
        />
        <MetricTile
          label={t("metrics.credits")}
          value={formatCents(creditCents)}
          subtitle={t("financeSummary.creditsSubtitle")}
          icon={ArrowDownLeft}
        />
        <MetricTile
          label={t("financeSummary.netLabel")}
          value={formatCents(netCents)}
          subtitle={t("financeSummary.netSubtitle")}
          icon={ReceiptText}
        />
        <MetricTile
          label={t("metrics.estimated")}
          value={formatCents(estimatedDebitCents)}
          subtitle={t("financeSummary.estimatedSubtitle")}
          icon={Coins}
        />
      </CardContent>
    </Card>
...
export function Costs() {
  ...
  const { t } = useTranslation(["navigation", "costs", "common"]);
      ...
      <div className="space-y-5">
          ...
          {preset === "custom" ? (
            <div className="flex flex-wrap items-center gap-2 border border-border p-3">
              ...
              />
              <span className="text-sm text-muted-foreground">{t("costs:customDateTo")}</span>
              <input
                type="date"
                value={customTo}
          ...
          <div className="grid gap-3 lg:grid-cols-4">
            <MetricTile
              ...
              subtitle={t("costs:topMetrics.inferenceSubtitle", { tokens: formatTokens(inferenceTokenTotal) })}
              icon={DollarSign}
            />
            <MetricTile
              ...
              value={activeBudgetIncidents.length > 0 ? String(activeBudgetIncidents.length) : (
                spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                  ...
                  : t("costs:budgetStatus.open")
              )}
              subtitle={
                activeBudgetIncidents.length > 0
                  ? t("costs:topMetrics.incidentSubtitle", {
                      agents: budgetData?.pausedAgentCount ?? 0,
                      projects: budgetData?.pausedProjectCount ?? 0,
                    })
                  : spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                    ? t("costs:topMetrics.budgetOfTotal", {
                        spent: formatCents(spendData.summary.spendCents),
                        total: formatCents(spendData.summary.budgetCents),
                      })
            ...
            <MetricTile
              ...
              subtitle={t("costs:topMetrics.financeNetSubtitle", {
            ...
            <MetricTile
              ...
              subtitle={t("costs:topMetrics.estimatedInRange", {
                    ...
                    <div className="flex flex-wrap items-end justify-between gap-3">
                      <div>
                        ...
                        <div className="mt-1 text-sm text-muted-foreground">
                          {spendData?.summary.budgetCents && spendData.summary.budgetCents > 0
                            ? t("costs:budgetDisplay", { amount: formatCents(spendData.summary.budgetCents) })
                            : t("costs:budgetStatus.unlimited")}
                        </div>
                      </div>
                ...
                <Card>
                  ...
                  <CardContent className="space-y-2 px-5 pb-5 pt-2">
                    {(spendData?.byAgent.length ?? 0) === 0 ? (
                      <p className="text-sm text-muted-foreground">{t("costs:byAgent.noCosts")}</p>
                    ) : (
                          ...
                          <div key={row.agentId} className="border border-border px-4 py-3">
                            ...
                            >
                              ...
                              <div className="text-right text-sm tabular-nums">
                                ...
                                <div className="text-xs text-muted-foreground">
                                  {t("costs:byAgent.inTokens", { tokens: formatTokens(row.inputTokens + row.cachedInputTokens) })} · {t("costs:byAgent.outTokens", { tokens: formatTokens(row.outputTokens) })}
                                </div>
                                {(row.apiRunCount > 0 || row.subscriptionRunCount > 0) ? (
                                  <div className="text-xs text-muted-foreground">
                                    {t("costs:byAgent.apiRunCount", { count: row.apiRunCount })}
                                    {" · "}
                                    {t("costs:byAgent.subscriptionRunCount", { count: row.subscriptionRunCount })}
                                  </div>
                                ) : null}
                              </div>
                            </div>
                                    ...
                                    >
                                      ...
                                      <div className="text-right tabular-nums">
                                        ...
                                        <div className="text-muted-foreground">
                                          {t("costs:byAgent.tokenCount", { tokens: formatTokens(modelRow.inputTokens + modelRow.cachedInputTokens + modelRow.outputTokens) })}
                                        </div>
                                      </div>
                                    </div>
                                  );
                  ...
                  <Card>
                    ...
                    <CardContent className="space-y-2 px-5 pb-5 pt-2">
                      {(spendData?.byProject.length ?? 0) === 0 ? (
                        <p className="text-sm text-muted-foreground">{t("costs:byProject.noCosts")}</p>
                      ) : (
            ...
            <>
              ...
              {activeBudgetIncidents.length > 0 ? (
                <div className="space-y-3">
                  <div>
                    <h2 className="text-lg font-semibold">{t("costs:budget.incidentsTitle")}</h2>
                    <p className="text-sm text-muted-foreground">
                      {t("costs:budget.incidentsDesc")}
                    </p>
                  </div>
                  <div className="grid gap-4 xl:grid-cols-2">
                    ...
                    <section key={scopeType} className="space-y-3">
                      <div>
                        <h2 className="text-lg font-semibold">
                          {scopeType === "company"
                            ? t("costs:budget.companyBudgets")
                            : scopeType === "agent"
                              ? t("costs:budget.agentBudgets")
                              : t("costs:budget.projectBudgets")}
                        </h2>
                        <p className="text-sm text-muted-foreground">
                          {scopeType === "company"
                            ? t("costs:budget.companyBudgetsDesc")
                            : scopeType === "agent"
                              ? t("costs:budget.agentBudgetsDesc")
                              : t("costs:budget.projectBudgetsDesc")}
                        </p>
                      </div>
                      <div className="grid gap-4 xl:grid-cols-2">
                        {rows.map((summary) => (
            ...
            <>
              <Tabs value={effectiveProvider} onValueChange={setActiveProvider}>
                ...
                <TabsContent value="all" className="mt-4">
                  {providers.length === 0 ? (
                    <p className="text-sm text-muted-foreground">{t("costs:noEvents_period")}</p>
                  ) : (
