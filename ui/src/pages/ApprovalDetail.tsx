...
export function ApprovalDetail() {
  ...
  const approveMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.approveFailed")),
  });
  ...
  const rejectMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.rejectFailed")),
  });
  ...
  const revisionMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.revisionFailed")),
  });
  ...
  const resubmitMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.resubmitFailed")),
  });
  ...
  const addCommentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.commentFailed")),
  });
  ...
  const deleteAgentMutation = useMutation({
    ...
    onError: (err) => setError(err instanceof Error ? err.message : t("errors.deleteFailed")),
  });
      ...
      ? {
          label:
            (linkedIssues?.length ?? 0) > 1
              ? t("banner.reviewLinkedIssues")
              : t("banner.reviewLinkedIssue"),
          to: `/issues/${primaryLinkedIssue.identifier ?? primaryLinkedIssue.id}`,
        }
      : linkedAgentId
        ? {
            label: t("banner.openHiredAgent"),
            to: `/agents/${linkedAgentId}`,
          }
        : {
            label: t("banner.backToApprovals"),
            to: "/approvals",
          };
  ...
  return (
    <div className="space-y-6 max-w-3xl">
        ...
        <div className="border border-green-300 dark:border-green-700/40 bg-green-50 dark:bg-green-900/20 rounded-lg px-4 py-3 animate-in fade-in zoom-in-95 duration-300">
          <div className="flex items-start justify-between gap-3">
            <div className="flex items-start gap-2">
              ...
              <div>
                <p className="text-sm text-green-800 dark:text-green-100 font-medium">{t("banner.approvalConfirmed")}</p>
                <p className="text-xs text-green-700 dark:text-green-200/90">
                  {t("banner.agentNotified")}
                </p>
              </div>
            </div>
            <Button
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
              ...
              <h2 className="text-lg font-semibold">{
                (() => {
                  const typeLabelKey = approval.type === "hire_agent" ? "typeLabels.hireAgent"
                    ...
                    : null;
                  const base = typeLabelKey ? t(typeLabelKey) : approval.type;
        ...
        <div className="text-sm space-y-1">
          ...
          {approval.decisionNote && (
            <p className="text-xs text-muted-foreground">{t("decisionNote", { note: approval.decisionNote })}</p>
          )}
        </div>
        {error && <p className="text-sm text-destructive">{error}</p>}
        {linkedIssues && linkedIssues.length > 0 && (
          <div className="pt-2 border-t border-border/60">
            <p className="text-xs text-muted-foreground mb-1.5">{t("linkedIssues.title")}</p>
        ...
        <div className="flex flex-wrap items-center gap-2">
          ...
          {isBudgetApproval && approval.status === "pending" && (
            <p className="text-sm text-muted-foreground">
              {t("budgetStop.resolveFrom")} <Link to="/costs" className="underline underline-offset-2">{t("budgetStop.costsPage")}</Link>.
            </p>
          )}
          ...
          {approval.status === "rejected" && approval.type === "hire_agent" && linkedAgentId && (
            ...
            >
              {t("actions.deleteDisapprovedAgent")}
            </Button>
          )}
        </div>
      </div>
      ...
      <div className="border border-border rounded-lg p-4 space-y-3">
        <h3 className="text-sm font-medium">{t("comments.title", { count: comments?.length ?? 0 })}</h3>
            ...
            <div key={comment.id} className="border border-border/60 rounded-md p-3">
              <div className="flex items-center justify-between mb-1">
                {comment.authorAgentId ? (
                  ...
                  <Identity name={t("comments.board")} size="sm" />
                )}
        ...
        <div className="flex justify-end">
          ...
          >
            {addCommentMutation.isPending ? t("actions.postingComment") : t("actions.postComment")}
          </Button>
        </div>
      </div>
    </div>
