...
export function buildRoutineGroups(
  ...
  t: (key: string) => string = (key) => key.split(":").pop()?.split(".").pop() ?? key,
): RoutineGroup[] {
  ...
  if (groupByValue === "project") {
    ...
    return Object.keys(groups)
      .sort((left, right) => {
        const leftLabel = left === "__no_project" ? t("noProject") : (projectById.get(left)?.name ?? t("unknownProject"));
        const rightLabel = right === "__no_project" ? t("noProject") : (projectById.get(right)?.name ?? t("unknownProject"));
        return leftLabel.localeCompare(rightLabel);
      })
      .map((key) => ({
        key,
        label: key === "__no_project" ? t("noProject") : (projectById.get(key)?.name ?? t("unknownProject")),
        items: groups[key]!,
      }));
  }
  ...
  return Object.keys(groups)
    .sort((left, right) => {
      const leftLabel = left === "__unassigned" ? t("unassigned") : (agentById.get(left)?.name ?? t("unknownAgent"));
      const rightLabel = right === "__unassigned" ? t("unassigned") : (agentById.get(right)?.name ?? t("unknownAgent"));
      return leftLabel.localeCompare(rightLabel);
    })
    .map((key) => ({
      key,
      label: key === "__unassigned" ? t("unassigned") : (agentById.get(key)?.name ?? t("unknownAgent")),
      items: groups[key]!,
    }));
}
      ...
      <div className="min-w-0 flex-1 space-y-1.5">
        <div className="flex flex-wrap items-center gap-2">
          ...
          {(isArchived || routine.status === "paused" || isDraft) ? (
            <span className="text-xs text-muted-foreground">
              {isArchived ? t("state.archived").toLowerCase() : isDraft ? t("state.draft").toLowerCase() : t("status.paused").toLowerCase()}
            </span>
          ) : null}
        </div>
        <div className="flex flex-wrap items-center gap-x-4 gap-y-1 text-xs text-muted-foreground">
          <span className="flex items-center gap-2">
            ...
            />
            <span>{routine.projectId ? (project?.name ?? t("unknownProject")) : t("noProject")}</span>
          </span>
          <span className="flex items-center gap-2">
            {agent?.icon ? <AgentIcon icon={agent.icon} className="h-3.5 w-3.5 shrink-0" /> : null}
            <span>{routine.assigneeAgentId ? (agent?.name ?? t("unknownAgent")) : t("noDefaultAgent")}</span>
          </span>
          <span>
      ...
      <div className="flex items-center gap-3" onClick={(event) => { event.preventDefault(); event.stopPropagation(); }}>
        <div className="flex items-center gap-3">
          ...
          <span className="w-12 text-xs text-muted-foreground">
            {isArchived ? t("state.archived") : isDraft ? t("state.draft") : enabled ? t("state.on") : t("state.off")}
          </span>
        </div>
        ...
        <DropdownMenu>
          ...
          <DropdownMenuContent align="end">
            ...
            >
              {runningRoutineId === routine.id ? t("actions.running") : t("actions.run")}
            </DropdownMenuItem>
            <DropdownMenuSeparator />
            <DropdownMenuItem
...
export function Routines() {
  ...
  const { t } = useTranslation(["navigation", "routines"]);
  ...
  const createRoutine = useMutation({
    ...
    onSuccess: async (routine) => {
      ...
      pushToast({
        title: t("routines:created.title"),
        body: routine.assigneeAgentId
          ? t("routines:created.withTrigger")
          : t("routines:created.noAgent"),
        tone: "success",
      });
  ...
  const updateRoutineStatus = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines:updateFailed.title"),
        body: mutationError instanceof Error ? mutationError.message : t("routines:updateFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const runRoutine = useMutation({
    ...
    onError: (mutationError) => {
      pushToast({
        title: t("routines:runFailed.title"),
        body: mutationError instanceof Error ? mutationError.message : t("routines:runFailed.body"),
        tone: "error",
      });
    },
  });
  ...
  const routineGroups = useMemo(
    () => buildRoutineGroups(routines ?? [], routineViewState.groupBy, projectById, agentById, tRoutines as (key: string) => string),
  ...
  function handleToggleEnabled(routine: RoutineListItem, enabled: boolean) {
    if (!enabled && !routine.assigneeAgentId) {
      pushToast({
        title: t("routines:agentRequired.title"),
        body: t("routines:agentRequired.body"),
        tone: "warn",
      });
      return;
    }
    ...
    <div className="space-y-6">
      ...
      <Tabs value={activeTab} onValueChange={handleTabChange}>
        <PageTabBar
          ...
          items={[
            { value: "routines", label: t("routines:rowTabs.routines") },
            { value: "runs", label: t("routines:rowTabs.runs") },
          ]}
        />
              ...
              <div className="overflow-x-auto overscroll-x-contain">
                <div className="inline-flex min-w-full flex-wrap items-center gap-2 text-sm text-muted-foreground sm:min-w-max sm:flex-nowrap">
                  ...
                  <InlineEntitySelector
                    ...
                    emptyMessage={t("routines:noAssignee")}
                  ...
                  <InlineEntitySelector
                    ...
                    emptyMessage={t("routines:noProjectOption")}
                    onChange={(projectId) => {
